...
function initAllCarousels() {
  ...
  carousels.forEach(function (carousel) {
     ...
     * @param {Event} e
     */
    // State for queuing rapid clicks
    const state = {
      queueNext: 0,
      queuePrev: 0,
      animating: false,
    ...
    /**
     * Process the queued carousel actions. If an animation is not
     * currently running and there are queued next or prev actions,
     * this function will start the appropriate animation. When the
     * animation completes, it will call processQueue() again to
     * handle additional queued actions.
     */
    function processQueue() {
      if (state.animating) return;
      if (state.queueNext > 0) {
        state.queueNext--;
        animateNext(processQueue);
      } else if (state.queuePrev > 0) {
        state.queuePrev--;
        animatePrev(processQueue);
      }
    }

    /**
     * Animate scrolling to the next card and rotate the first card
     * to the end. Accepts a callback that is invoked after the
     * repositioning is complete.
     * @param {Function} done
     */
    function animateNext(done) {
      ...
      // reposition scrollLeft instantly (without animation). Use
      // setTimeout matching the animation duration (~350ms).
      setTimeout(function () {
    ...
    /**
     * Animate scrolling to the previous card and rotate the last card
     * to the beginning. Accepts a callback that is invoked after
     * repositioning completes.
     * @param {Function} done
     */
    function animatePrev(done) {
      const cardWidth = getCardWidth();
      if (!cardWidth) {
        if (done) done();
        return;
      ...
      if (last) {
        const originalSnap = container.style.scrollSnapType;
        const originalBehavior = container.style.scrollBehavior;
        // Prepare by inserting the last card at the beginning and
        // shifting scrollLeft forward by one card width.
        container.style.scrollSnapType = 'none';
        container.style.scrollBehavior = 'auto';
        container.insertBefore(last, container.firstElementChild);
        container.scrollLeft += cardWidth;
        requestAnimationFrame(function () {
          // Restore snapping and animate back
          container.style.scrollSnapType = originalSnap || '';
          container.style.scrollBehavior = originalBehavior || '';
          container.scrollBy({ left: -cardWidth, behavior: 'smooth' });
          // Wait for the scroll back animation to finish, then mark done
          setTimeout(function () {
            state.animating = false;
            if (done) done();
          }, 350);
    ...
    // Queue up next/prev actions rather than executing immediately. This
    // allows smooth handling of rapid clicks without disrupting the
    // animation. Each click increments a counter and triggers
    // processQueue().
